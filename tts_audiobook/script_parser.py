...
def parse_script_llm(
    ...
    # 模糊匹配：修正 speaker 名称为角色卡正式名
    for seg in segments:
        name = seg.get("speaker", "旁白")
        if name not in char_map and name != "旁白":
...
def _split_paragraph_by_speaker(
    ...
    for q_start, q_end, quote_text in sorted(quote_spans):
        ...
        if before:
            result.append({"speaker": "旁白", "text": before})
        ...
        result.append({"speaker": speaker, "text": quote_text})
    ...
    if after:
        result.append({"speaker": "旁白", "text": after})

    return result if result else [{"speaker": "旁白", "text": para}]
...
def parse_script(
    ...
    # 模糊匹配：修正说话人名称为角色卡中的正式名称
    char_names = {c["name"] for c in characters}
    for seg in segments:
        name = seg.get("speaker", "旁白")
        if name != "旁白" and name not in char_names:
            matched = _fuzzy_match_character(name, {n: {} for n in char_names})
            if matched:
                seg["speaker"] = matched
